...
@section('page-title', 'Layanan Rumah Sakit')
...
<a href="{{ route('admin.layanan.index') }}" class="flex items-center px-6 py-3 text-gray-700 bg-gray-100 border-l-4 border-blue-600"><span>⚕️ Layanan</span></a>
...
<div class="grid md:grid-cols-2 gap-6">
    <div class="bg-white rounded-lg shadow-sm p-6">
        <h3 class="text-lg font-semibold text-gray-800 mb-4">Tambah Layanan</h3>
        <form method="POST" action="{{ route('admin.layanan.store') }}">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="price_range" value="{{ old('price_range') }}" placeholder="cth: Rp 50.000 - Rp 200.000" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
            <div class="mb-4">
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <h3 class="text-lg font-semibold text-gray-800 mb-4">Daftar Layanan ({{ $services->count() }})</h3>
        <div class="space-y-3 max-h-96 overflow-y-auto">
            ...
            <div class="border border-gray-200 rounded-lg p-3">
                <div class="flex justify-between items-start">
                    <div>
                        ...
                        @if($s->department)<div class="text-xs text-blue-600 mt-1">{{ $s->department->name }}</div>@endif
                        @if($s->price_range)<div class="text-sm text-green-600 mt-1">{{ $s->price_range }}</div>@endif
                        <span class="text-xs {{ $s->is_active ? 'text-green-600' : 'text-red-500' }}">{{ $s->is_active ? 'Aktif' : 'Nonaktif' }}</span>
                    </div>
                    <form method="POST" action="{{ route('admin.layanan.destroy', $s->id) }}" onsubmit="return confirm('Hapus?')">
                        @csrf @method('DELETE')
            ...
            @empty
            <p class="text-gray-500 text-center py-4">Belum ada layanan</p>
            @endforelse
        </div>
    </div>
</div>
